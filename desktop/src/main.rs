...
fn main() {
    ...
    let rom = std::env::args().nth(1).map(|s| {
        std::fs::read(&s).unwrap_or_else(|e| panic!("Could not read ROM file {s} with error: {e}"))
    });
...
impl EmulatorPlatform for Desktop {
    fn create_window(proxy: &Arc<EventLoopProxy<EmulatorEvent>>, event_loop: &ActiveEventLoop) {
        let window = event_loop
            .create_window(Window::default_attributes().with_title("Gameboy Emulator"))
            .unwrap_or_else(|e| panic!("Could not create window with error: {e}"));
